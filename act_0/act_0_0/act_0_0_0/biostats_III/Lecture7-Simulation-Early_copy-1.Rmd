...
```{r slr,fig.height=3,fig.width=6}
...
for(i in 1:K){
  # Generate X = explanatory variable
  X = runif(n)
  # Generate Y = mean + error
  # The model below has assumptions that hold under SLR
  # y = 2 + 0.25 * X + rnorm(n,sd=0.2)
  # Specify a model that violates the SLR assumptions
  y = 2 + 0.25 * X + scale(rchisq(n,df=1))
  # Fit the SLR and save the slope
  fit = lm(y~X)
  slopes = c(slopes,fit$coefficients[2])
}

par(mfrow=c(1,2),mar=c(4,4,1,1))
hist(slopes, breaks=20, prob=TRUE, 
     xlab="Estimated slope (n=30)", 
     main="Hist + normal curve")
curve(dnorm(x, mean=mean(slopes), sd=sqrt(var(slopes))), 
      col="darkblue", lwd=2, add=TRUE, yaxt="n")
qqnorm(scale(slopes));abline(0,1)
```